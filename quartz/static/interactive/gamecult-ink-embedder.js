;(function () {
  ...
  function resolveInkMode(container) {
    const rawMode = (container.dataset.inkFormat || container.dataset.inkMode || "raven")
      .trim()
      .toLowerCase()

    if (["visual-novel", "visual_novel", "vn", "speaker", "speaker-panel"].includes(rawMode)) {
  ...
  function renderChoices(story, choices, variables) {
    ...
    if (story.currentChoices.length === 0) {
      choices.append(
        createElement("p", "aetheria-ink-end", "End of branch. The feed stops blinking."),
      )
      return
    }
    ...
    story.currentChoices.forEach((choice, index) => {
      ...
      button.addEventListener("click", () => {
        ...
        appendParagraphs(
          story,
          choices.closest(".aetheria-ink-player").querySelector(".aetheria-ink-transcript"),
        )
  ...
  function renderSpeakerStep(story, state) {
    ...
    if (story.currentChoices.length === 0) {
      choices.append(
        createElement("p", "aetheria-ink-end", "End of branch. The circle goes quiet."),
      )
      return
    }
